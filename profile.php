...
{
    $stmt = $con->prepare("UPDATE create_account SET name=?, password=?, mobile=?, address=? WHERE email=?");
    $stmt->bind_param("sssss", $_POST['name'], $_POST['pass'], $_POST['mob'], $_POST['add'], $eid);
    $stmt->execute();
      ...
      <div class="form-group">
           <div class="row">
              <div class="control-label col-sm-4"><h4>Gender:</h4></div>
                <div class="col-sm-8">
                <h4> <strong><?php echo $result['gender']; ?></strong></h4>
          </div>
        </div>
      </div>
