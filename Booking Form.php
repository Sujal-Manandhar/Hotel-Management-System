...
$stmt = $con->prepare("SELECT * FROM room_booking_details WHERE email=?");
...
if (isset($_POST['savedata'])) {
    ...
    if ($res2->num_rows > 0) {
        ...
        // For simplicity, defining empty string if zip is missing since the old form doesn't seem to pass zip, state, country?
        $state = $_POST['state'] ?? '';
        $zip = $_POST['zip'] ?? '';
